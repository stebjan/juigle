...
public class PostgreSQLDBSupport extends DBSupport {
	...
	public PostgreSQLDBSupport(DatabaseInfo databaseInfo, DataSource dataSource,
	...
	public Set<String> getSequenceNames(String schemaName) {
		...
		// Should be replaced by the original query on information_schema.sequences
		// in future, since this is a more elegant solution
		// This is the original query:
		// getItemsAsStringSet("select sequence_name from information_schema.sequences where sequence_schema = '"
		// + schemaName + "'", getDataSource());
		return getSQLHandler()
		    .getItemsAsStringSet(
		        "select c.relname from pg_class c join pg_namespace n on (c.relnamespace = n.oid) where c.relkind = 'S' and n.nspname = '"
		            + schemaName + "'", getDataSource());
	}
	 ...
	 * <p/>
	 * The drop trigger statement is not compatible with standard SQL in
	 * Postgresql. You have to do drop trigger 'trigger-name' ON 'table name'
	 * instead of drop trigger 'trigger-name'.
	 * <p/>
	...
	public Set<String> getTriggerNames(String schemaName) {
		...
		Set<String> triggerAndTableNames = getSQLHandler()
		    .getItemsAsStringSet(
	...
	/**
	 * Drops the sequence with the given name from the database Note: the sequence
	 * name is surrounded with quotes, making it case-sensitive.
	 * <p/>
	 * The method is overriden to handle columns of type serial. For these
	 * columns, the sequence should be dropped using cascade. Thanks to Peter
	 * Oxenham for reporting this issue (UNI-28).
	 * 
	 * @param sequenceName
	...
	public void dropSequence(String schemaName, String sequenceName) {
		getSQLHandler().executeUpdate(
		    "drop sequence " + qualified(schemaName, sequenceName) + " cascade",
		    getDataSource());
	}
	 ...
	 * 
	 * @param triggerName
	 *          The trigger to drop as 'trigger-name' ON 'table name', not null
	 */
	@Override
	...
	public Set<String> getTypeNames(String schemaName) {
		return getSQLHandler()
		    .getItemsAsStringSet(
		        "select object_name from information_schema.data_type_privileges where object_type = 'USER-DEFINED TYPE' and object_schema = '"
		            + schemaName + "'", getDataSource());
	}
	...
	protected void disableReferentialConstraints(String schemaName,
		...
		Set<String> constraintNames = sqlHandler
		    .getItemsAsStringSet(
		        "select constraint_name from information_schema.table_constraints con where con.table_name = '"
		            + tableName
		            + "' and constraint_type = 'FOREIGN KEY' and constraint_schema = '"
		            + schemaName + "'", getDataSource());
		for (String constraintName : constraintNames) {
			sqlHandler.executeUpdate("alter table "
			    + qualified(schemaName, tableName) + " drop constraint "
	...
	protected void disableValueConstraints(String schemaName, String tableName) {
		...
		// The join wiht pg_constraints is used to filter out not null
		// check-constraints that are implicitly created by Postgresql
		Set<String> constraintNames = sqlHandler
		    .getItemsAsStringSet(
		        "select constraint_name from information_schema.table_constraints con, pg_constraint pg_con where pg_con.conname = con.constraint_name and con.table_name = '"
		            + tableName
		            + "' and constraint_type in ('CHECK', 'UNIQUE') and constraint_schema = '"
		            + schemaName + "'", getDataSource());
		for (String constraintName : constraintNames) {
			sqlHandler.executeUpdate("alter table "
			    + qualified(schemaName, tableName) + " drop constraint "
		...
		// retrieve the name of the primary key, since we cannot remove the not-null
		// constraint on this column
		Set<String> primaryKeyColumnNames = sqlHandler
		    .getItemsAsStringSet(
		        "select column_name from information_schema.table_constraints con, information_schema.key_column_usage key where con.table_name = '"
		            + tableName
		            + "' and con.table_schema = '"
		            + schemaName
		            + "' and key.table_name = con.table_name and key.table_schema = con.table_schema and key.constraint_name = con.constraint_name and con.constraint_type = 'PRIMARY KEY'",
		        getDataSource());
		...
		for (String notNullColumnName : notNullColumnNames) {
			...
			sqlHandler.executeUpdate("alter table "
			    + qualified(schemaName, tableName) + " alter column "
			    + notNullColumnName + " drop not null", getDataSource());
		}
	}
	...
	public long getSequenceValue(String schemaName, String sequenceName) {
		return getSQLHandler().getItemAsLong(
		    "select last_value from " + qualified(schemaName, sequenceName),
		    getDataSource());
	}
	...
	public void incrementSequenceToValue(String schemaName, String sequenceName,
		...
		getSQLHandler().getItemAsLong(
		    "select setval('" + qualified(schemaName, sequenceName) + "', "
		        + newSequenceValue + ")", getDataSource());
	}
	...
	public void setDatabaseDefaultSchema() {
		getSQLHandler().executeUpdate(
		    "SET search_path TO " + getDefaultSchemaName(), getDataSource());
	}
